...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Joao\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5548991635455:+55 48991635455\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[✘] ESTE PEDIDO É ESPECÍFICO PARA USUARIO DO JOAO BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo do joao. Registre-se para fazer amizade com o bot Joao por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar joao|19\n\n──「 Joao BOT 」──`,
			...
			const premium = ["5548991635455@s.whatsapp.net","559784130980@s.whatsapp.net","558381572957@s.whatsapp.net]"
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Nesse grupo, não gostamos de racismos...")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/joaojokes.js');
...
//Joao
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('onde está o emojis?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('5548991635455@s.whatsapp.net', options, text, {quoted: mek})
		   ...
		   await limitAdd(sender)
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('afk por causa do que?')                                  
...
// recursos anime, random @joao domina
			...
			break
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'party':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=ANTIGRATISNIHANJENKKK`)
					client.sendMessage(from, buffer, image, {caption: 'Aqui man', quoted: mek})
					break
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí Joao, chama ele aí ele"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT JOAO* 」\n*│+ wa.me/5548991635455*╰──────*「 *JOAO* 」*────*\n\n*__*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do joao:*\n\n OFF'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
